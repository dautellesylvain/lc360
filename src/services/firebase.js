...
export const signInWithGoogle = async () => {
  ...
  if (!snap.exists()) {
    ...
    // Mettre à jour la locale à chaque connexion
    await updateDoc(userRef, { locale, lastLoginAt: serverTimestamp() })
  }
  return cred
}
...
export const deleteProject = async (projectId) => {
  // Supprimer les analytics (sous-collection visits)
  try {
    const visitsRef = collection(db, 'analytics', projectId, 'visits')
    const visitsSnap = await getDocs(visitsRef)
    await Promise.all(visitsSnap.docs.map(d => deleteDoc(d.ref)))
    await deleteDoc(doc(db, 'analytics', projectId))
  } catch(e) {}
  // Supprimer le projet
...
export const getProjectByToken = async (token) => {
  ...
  if (project.ownerId) {
    try {
      ...
      if (ownerSnap.exists()) {
        project.ownerPlan = ownerSnap.data().plan || 'free'
      }
    } catch(e) {}
  }
...
/** Vérifie les milestones de visiteurs et met à jour Firestore */
export const checkVisitorMilestones = async (projectId, ownerId) => {
  try {
    const visitsSnap = await getDocs(collection(db, 'analytics', projectId, 'visits'))
    const totalVisitors = new Set(visitsSnap.docs.map(d => d.data().visitorId)).size

    const ownerSnap = await getDoc(doc(db, 'users', ownerId))
    const celebrations = ownerSnap.data()?.celebrations || {}

    const milestones = []

    // 100 visiteurs uniques
    if (totalVisitors >= 100 && !celebrations.hundredVisitors) {
      milestones.push('hundredVisitors')
    }

    if (milestones.length > 0) {
      const updates = {}
      milestones.forEach(m => { updates[`celebrations.${m}`] = new Date().toISOString() })
